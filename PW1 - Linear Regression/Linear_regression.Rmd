...
### 3) Relationship between age and medv
...
```
There is a weak linear relationship between the predictor age and the response medv because the coefficient of correlation is closer to 0.  
...
### 4) Fitting a housing prices model in function of age
...
```
As infer above, there is a correlation but the variabilty is important

### 5) Train a more complex model
...
PS: We can't do the same transformation for the age predictor because the weak linearship relation comes from the data variability 

```{r}
model_age_lstat = lm(medv~age+lstat, data=training_data)
model_age_lstat
```
...
### 7) Whole significant
We use the Fisher test that was performed by R
